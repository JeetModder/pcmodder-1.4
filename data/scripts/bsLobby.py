...
class Chooser(object):
    ...
    def __init__(self, vPos, player, lobby):
        ...
        bs.screenMessage(u"\ue043 Welcome to server by Jeet! \ue043\n Make sure to Join Discord!", color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)), clients=[clID], transient=True)
        ...
        import fire
        if fire.whitelist:    #by PCModder
            if profile not in MID.whitelist:
               clID = self._player.getInputDevice().getClientID() 
               name = self._player.getName()
               bs.screenMessage('Account not Whitelisted sorry Contact PCModder\n Server kicking '+profile,color=(1,1,1), clients=[clID], transient=True)
               def kick():
                   bsInternal._disconnectClient(clID)
               bs.gameTimer(30,bs.Call(kick)) 

        if profile not in MID.verify:#created by PCModder
            if settings.enableVerification:    
               ...
               def wow():
                   bsInternal._disconnectClient(clID)
                   bs.screenMessage('Account has not been Verified\n Server Kicking Player '+profile,color=(1,1,1), clients=[clID], transient=True)
               bs.gameTimer(31000,bs.Call(wow))
